...
is called $q$-\emph{Lipschitz}, where $q > 0$, 
if $
...
We define the representation $\deltaboxCM^1$ of $C$ as follows: 
a \emph{$\deltaboxCM^1$-name} of $g \in C$
is a pair $\langle \phi, q \rangle$ such that 
$\phi \in \LM$ is a $\deltabox$-name of $g$ 
and $q$ is a dyadic number such that $g$ is $q$-Lipschitz. 
\begin{lemma}
\label{lemma:P-hard-g_u}
$\OpCMFix$ is $(\deltaboxCM^1, \rhoR)$-$\classFPtwo$-$\redLB$-complete, 
even if $\dom \OpCMFix$ is restricted to contractions 
...
This will be proved in the appendix using Lemma~\ref{lemma:P-hard-g_u}. 
