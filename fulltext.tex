...
by extending the \emph{stack model with constant height} 
by Aehlig, Cook and Nguyen 
\cite{aehlig2007relativizing},
since it is consistent with relativized circuit complexity classes 
and it makes some elemental operation log-space computable.
See the comment on Theorem~\ref{theorem:inclusion}. 
%% そのcommentが未了
In their paper, oracles are length monotone. 
%% ??
Hence, we add a read-only tape on which the oracle writes answers.
%% ??
The answer tape is erased when a machine \emph{pushes},
then it starts writing a new query tape.
%% ??
